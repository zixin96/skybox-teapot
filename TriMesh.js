 ...
 */
    ...
    */
    computeAABB(){
        ...
        var temp = 2;
        var temp2 = 1;
        for(let i = 0; i < this.vBuffer.length / 3; i++){
            dataX.push(this.vBuffer[3*i]);
        }
        
        for(let i = 0; i < this.vBuffer.length / 3; i++){
            dataY.push(this.vBuffer[temp2]);
            temp2 += 3;
        }
        
        for(let i = 0; i < this.vBuffer.length / 3; i++){
            dataZ.push(this.vBuffer[temp]);
            temp += 3;
        }
        
        this.minXYZ = [Math.min(...dataX), Math.min(...dataY), Math.min(...dataZ)];
        this.maxXYZ = [Math.max(...dataX), Math.max(...dataY), Math.max(...dataZ)];
    }
    ...
    */
    loadFromOBJ(fileText)
    {
        ...
        // x+ Matches the preceding x one or more times 
        //Your code here
        
        //v followed by a whitespace
        const VERTS = /^v\s/;
        //f followed by a whitespace
        const FACES = /^f\s/;
        //Whitespace followed by one or more whitespace
        const WHITESPACE = /\s+/;
        
        //Extract every single line in OBJ. These lines were sperated by the newline 
        const lines = fileText.split("\n");
        
        for(let i = 0; i < lines.length; i++){
            //In case there are whitespace characters on both ends of a line 
            const line = lines[i].trim();
            //We don't process comments or invalid lines 
            if(line.startsWith("#") || !line){
                continue;
            }
            //Extract our vertices/faces elements 
            const data = line.split(WHITESPACE);
            //Delete v/f
            data.shift();
            
            
            if(VERTS.test(line)){
                this.vBuffer.push(...data);
                
            }
            
            else if(FACES.test(line)){
                this.fBuffer.push(data[0] - 1);
                this.fBuffer.push(data[1] - 1);
                this.fBuffer.push(data[2] - 1);
            }   
        }
